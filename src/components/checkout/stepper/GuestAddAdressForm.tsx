...
export const GuestAddAdressForm: FC<{
  ...
  const isEmailFilled = email && email !== "" && typeof email !== "object";
    ...
    return (
      <div className="mt-4 p-4 bg-neutral-100 dark:bg-neutral-800 rounded-lg">
        <p className="text-base text-neutral-600 dark:text-neutral-400">
          Please enter your email address first to proceed with the checkout.
        </p>
      </div>
    );
  }
  ...
  const extractLast10 = (phone: string) => phone.replace(/\D/g, "").slice(-10);

  const addGuestAddress = async (data: any) => {
    const billing = { ...data?.billing, phone: extractLast10(data?.billing?.phone) };
    const shipping = data?.shipping
      ? { ...data.shipping, phone: extractLast10(data.shipping.phone) }
      : data?.shipping;
