...
export async function generateMetadataForPage(
  ...
  const canonicalUrl =
    seo.canonical || fallback?.canonical || `${process.env.NEXTAUTH_URL}/${slug}`;
  ...
  return {
    metadataBase: new URL(baseUrl || process.env.NEXTAUTH_URL || "http://localhost:3000"),
...
export const calculateAverageRating = (reviews: any): number => {
    if (!reviews?.edges || reviews.edges.length === 0) return 0;
    const sum = reviews.edges.reduce((acc: number, edge: any) => acc + (edge?.node?.rating || 0), 0);
    return parseFloat((sum / reviews.edges.length).toFixed(1));
};
