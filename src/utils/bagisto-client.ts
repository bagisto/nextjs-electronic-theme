...
import { GET_CUSTOMER_PROFILE } from "@/graphql/customer/queries";
import { DocumentNode } from "graphql";
import { BAGISTO_SESSION, GRAPHQL_URL, HIDDEN_PRODUCT_TAG, STOREFRONT_KEY } from "@/utils/constants";
...
/**
 * Fetches the `shipping_info` theme customization and returns a normalised array
 * of `{ icon, title, subtitle }` entries. Returns an empty array when the
 * customization is missing or unparseable so the UI can fall back to its
 * built-in defaults.
 */
export async function getShippingInfo(): Promise<ShippingInfoItem[]> {
  try {
    const res = await bagistoFetch<{
      ...
      query: GET_FOOTER,
      variables: { type: "shipping_info" },
      isCookies: false,
      revalidate: 86400,
    });

    const edges = res.body.data?.themeCustomizations?.edges ?? [];
    if (!edges.length) return [];

    const node = edges[0]?.node;
    const translation =
      node?.translations?.edges?.find((e) => e?.node?.locale === "en")?.node ??
      node?.translations?.edges?.[0]?.node;
    if (!translation) return [];

    let parsed: unknown = translation.options;
    ...
    const candidate =
      (parsed as any)?.items ??
      (parsed as any)?.shipping_info ??
      (parsed as any)?.shipping ??
    ...
    return candidate
      .map((raw: any) => ({
        icon: raw?.icon ?? raw?.service_icon ?? undefined,
        title: raw?.title ?? raw?.label ?? raw?.name ?? "",
        subtitle: raw?.subtitle ?? raw?.description ?? "",
      }))
      .filter((item: ShippingInfoItem) => item.title.length > 0);
  } catch (err) {
    console.error("getShippingInfo Error:", err);
    return [];
  }
}
...
export async function updateCustomerProfile(input: UpdateCustomerProfileInput) {
  try {
    const res = await bagistoFetch<{
      data: {
    ...
    return res.body.data?.createCustomerProfileUpdate?.customerProfileUpdate;
  } catch (error) {
